...
| **B2 (#170)** | IBus signal body marshalling(`proxy.rs` 5 method を実 D-Bus signal emit に置換、`IBusText` / `IBusLookupTable` wire format 確定:`(sa{sv}sv)` / `(sa{sv}uubbiavav)`、IBus 1.5.x source 準拠、PR review 5 dim 統合反映で types を `attrs: IBusAttrList` / `candidates: Vec<IBusText>` に refactor、structural assertion test、`pub(crate)` visibility、threat model rustdoc) | **進行中**(本 PR、test **535 / 546**(+31)、release stub symbol 0、`KOTOHA_ALLOW_STUB=1` exit 1) |
